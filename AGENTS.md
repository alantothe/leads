...
- `apps/api/app/` hosts the FastAPI application; `apps/api/app/main.py` is the entrypoint.
- `apps/api/features/` contains feature modules; `apps/api/features/feed/` is split into `api/` (routes), `service/` (parsing logic), and `schema/` (Pydantic models).
- `apps/api/utils/` and `apps/api/lib/` hold shared helpers such as URL validation and date normalization.
...
- `pip install -r apps/api/requirements.txt` installs FastAPI and feedparser dependencies.
- `bun install` installs Turbo tasks for workspace scripts.
- `bun run dev` runs `turbo run dev`, which starts the API via `uvicorn`.
- `cd apps/api && python3 -m uvicorn app.main:app --reload` is the direct local server command (default `http://127.0.0.1:8000`).
...
- Keep API schema definitions in Pydantic models under `apps/api/features/**/schema/`.
- Validate external inputs in `apps/api/utils/` before calling services.
